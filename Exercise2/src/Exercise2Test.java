...
class Exercise2Test {
	...
	private final SearchOperations sut = new Searcher(new Database().getRecordings());
	private final Stats stats = new Searcher(new Database().getRecordings());
	...
	void __version() {
		System.out.println("Test version 1.10 (2021-04-10)");
	}
	...
	void testGetRecordingByNameWhenRecordingExists() {
		...
		assertEquals(TITLE_GIANT_STEPS, recording.get().getTitle());
	}
	...
	void testGetRecordingByNameWhenRecordingIsMissing() {
		...
		assertFalse(recording.isPresent());
	}
	...
	void testNumberOfArtists() {
		var actual = stats.numberOfArtists();
	...
	void testNumberOfGenres() {
		var count = stats.numberOfGenres();
	...
	void testNumberOfTitles() {
		var count = stats.numberOfTitles();
	...
	void testOfferHasNewRecordings() {
		...
		var gs = sut.getRecordingByName(TITLE_GIANT_STEPS).get();

		var offered = List.of(
				new Recording(TITLE_GIANT_STEPS, ARTIST_JOHN_COLTRANE, 1959, TYPE_LP, Set.of(GENRE_JAZZ)),
				punisher
		);
